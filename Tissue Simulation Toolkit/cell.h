...
public:
  ...
  */
  ...
  Cell(void) {
    ...
    Links = new bool[3000]; //Give every cell the array bool, which is large to avoid difficulties when new cells are created.
  };
  ...
  Cell(const Cell &src) {
    ...
    //Newly born cells start without filopodia
    Links = new bool[3000];
  ...
  */
  inline Cell &operator=(const Cell &src) {
    ...
    orientation=src.orientation;
    //Newly born cells start without filopodia
    Links = new bool[1000]; //initilalize links to empty
  ...
  */
  inline double TargetLength() const {
	...
	inline double getCenterX(void) {
     return sum_x/(double) area;
  }
  inline double getCenterY(void) {
     return sum_y/(double) area;
	}
...
protected:
  ...
  int tau; // Cell type, when dynamicJ's are not used
	double orientation; //This gives the orientation of the cell as a value between 0 and 2pi
