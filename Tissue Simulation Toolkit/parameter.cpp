...
Parameter::Parameter() {
  ...
  datadir = strdup("data_film");
	...
	infl_redonyellow = 1;
	infl_yellowonred = 1;
	infl_yellowonyellow = 1;

	mean_divtime_epi = 80000000;
	mean_divtime_meso = 200000000;
	begin_diff = 10000000;
	end_diff = 150000;
	mean_difftime = 400000;
...
void Parameter::Read(const char *filename) {
  ...
  target_length = igetpar(fp, "target_length", 10, true);
  lambda = fgetpar(fp, "lambda", 5, true);
  lambda2 = fgetpar(fp, "lambda2", 5, true);
  Jtable = sgetpar(fp, "Jtable", "J.dat", true);
  conn_diss = igetpar(fp, "conn_diss", 0, true);
  vecadherinknockout = bgetpar(fp, "vecadherinknockout", false, true);
  extensiononly = bgetpar(fp, "extensiononly", false, true);
  chemotaxis = igetpar(fp, "chemotaxis", 0, true);
  border_energy = igetpar(fp, "border_energy", 100, true);
  neighbours = igetpar(fp, "neighbours", 2, true);
  periodic_boundaries = bgetpar(fp, "periodic_boundaries", true, true);
  ...
  sizex = igetpar(fp, "sizex", 500, true);
  sizey = igetpar(fp, "sizey", 500, true);
  divisions = igetpar(fp, "divisions", 0, true);
  mcs = igetpar(fp, "mcs", 100000, true);
  rseed = igetpar(fp, "rseed", -1, true);
  subfield = fgetpar(fp, "subfield", 8.0, true);
  relaxation = igetpar(fp, "relaxation", 100000000, true);
  storage_stride = igetpar(fp, "storage_stride", 200, true);
	conv_ext = bgetpar(fp, "conv_ext", true, true);
  graphics = bgetpar(fp, "graphics", true, true);
  store = bgetpar(fp, "store", true, true);
  datadir = sgetpar(fp, "datadir", "data_film", true);

	max_links = igetpar(fp, "max_links", 3, true);
	theta_max = igetpar(fp, "theta_max", 45, true);
	r_max = igetpar(fp, "r_max", 2, true);
	r_max = r_max * target_length;
	lambda_force = igetpar(fp, "lambda_force", 50, true);
	t_interval = igetpar(fp, "t_interval", 20, true);

	memory = fgetpar(fp, "memory", 0.99, true);
	infl_redonred = fgetpar(fp, "infl_redonred", 1, true);
	infl_redonyellow = fgetpar(fp, "infl_redonyellow", 1, true);
	infl_yellowonred  = fgetpar(fp, "infl_yellowonred", 1, true);
	infl_yellowonyellow = fgetpar(fp, "infl_yellowonyellow", 1, true);

	mean_divtime_epi = igetpar(fp, "mean_divtime_epi", 80000000, true);
	mean_divtime_meso = igetpar(fp, "mean_divtime_meso", 200000000, true);
	begin_diff = igetpar(fp, "begin_diff", 10000000, true);
	end_diff = igetpar(fp, "end_diff", 150000, true);
	mean_difftime = igetpar(fp, "mean_difftime", 400000, true);
	decrease_size_div = bgetpar(fp, "decrease_size_div", true, true);

	frac_yellow = fgetpar(fp, "frac_yellow", 0.5, true);

	pulling_method = igetpar(fp, "pulling_method", 1, true);
}
