...
//This function intializes the coupled arrays edgelist and orderededgelist to enhance the speed of TST.
void CellularPotts::InitializeEdgeList(void){
	...
	sizeedgelist = 0; //Keeps track of how many edges may result in a different configuration
	int pixel;
	int neighbour;
	int x, y;
	int xp, yp;
	int c, cp;
  //Fill both array with -1 first.
	for (int k=0; k< (par.sizex-2) * (par.sizey-2) * nbh_level[par.neighbours]; k++){
		edgelist[k] = -1;
		orderedgelist[k] = -1;
	}
	for (int k=0; k< (par.sizex-2) * (par.sizey-2) * nbh_level[par.neighbours]; k++){
    //Calculate which lattice site and which neighbour we are looking at for k.
    ...
    //Make sure periodice boundaries are handled properly	
      ...
      cp=sigma[xp][yp];
			...
			cp=sigma[xp][yp];
    //If two sites are part of a different cell, add this edge to the edgelist and to orderededgelist.
    if (cp != c && cp != -1){				
...
//This function computes the degree of mixing
double CellularPotts::DegreeOfMixing(void){
	int TotalSurface = 0;
...
//This function will attach links to all cells. It will randomly attach at most par.max_links links at random to cells within a cone determined by par.r_max and par.theta_max.
void CellularPotts::RefreshLinks(void){ 
  ...
  //Count the number of cells
    ...
    //Get the center and polarization of cell c
		centX_1 = c->getCenterX(); 
		centY_1 = c->getCenterY(); 
		polX = cos(c->orientation);
		polY = sin(c->orientation);
	
		for (;z!=cell->end();z++) {
			switch(par.pulling_method){ 
//The switch corresponds to different methods of pulling: 
//1 describes cells pulling on the same cell type.
//2 describes all cells pulling on each other
//3 describes yellow cells pulling on all cells.
				case 1: 
					if(z->tau ==1){ 
            //Get the center and polarization of cell z
						centX_2 = z->getCenterX();
						centY_2 = z->getCenterY(); 
						deltaX = centX_2 - centX_1;
						deltaY = centY_2 - centY_1;
						c->Links[z->Sigma()] = false;
            //If cell z is within the cone of cell c and both have cell type 1, attach a link.
            ...
            //Get the center and polarization of cell z
						...
						deltaY = centY_2 - centY_1;
						c->Links[z->Sigma()] = false;
            //If cell z is within the cone of cell c and both have cell type 2, attach a link.
          ...
          //Get the center and polarization of cell z
					...
					deltaY = centY_2 - centY_1;
					c->Links[z->Sigma()] = false;
          //If cell z is within the cone of cell c, attach a link.
          ...
          //Get the center and polarization of cell z
					...
					deltaY = centY_2 - centY_1;
					c->Links[z->Sigma()] = false;
          //If cell c has cell type 2 and cell z is within the cone of cell c, attach a link       
					if((sqrt(deltaX*deltaX  + deltaY*deltaY)< par.r_max) && (c->tau == 2) && 
						((acos((deltaX*polX + deltaY*polY)/ (sqrt (deltaX*deltaX  + deltaY*deltaY) * sqrt (polX*polX  + polY*polY))) < par.theta_max*PI/180)||
						(PI-acos((deltaX*polX + deltaY*polY)/ (sqrt (deltaX*deltaX  + deltaY*deltaY) * sqrt (polX*polX  + polY*polY)))  < par.theta_max*PI/180))){
						number_of_links++;
    ...
    //randomly remove links until you only have n_max left
		...
		//Adjust orientations according to your neighbours, taking into account the memory parameter.
		if (number_of_links){
			z=cell->begin(); ++z;
			double AvgOrientation = 0;
			double AvgX = 0;
			double AvgY = 0;
      //For all cells, compute the average orientation of the neighbour, taking into account possible different influencess for different cell types.
			for (;z!=cell->end();z++){
				if (c->Links[z->Sigma()] && c->tau == 1 && z->tau == 1){ 
					AvgX+= cos(z->orientation)*par.infl_redonred;
					AvgY+= sin(z->orientation)*par.infl_redonred;
				}
				else if (c->Links[z->Sigma()] && c->tau == 1 && z->tau == 2){ 
					AvgX+= cos(z->orientation)*par.infl_yellowonred;
					AvgY+= sin(z->orientation)*par.infl_yellowonred;
				}
				else if (c->Links[z->Sigma()] && c->tau == 2 && z->tau == 1){ 
					AvgX+= cos(z->orientation)*par.infl_redonyellow;
					AvgY+= sin(z->orientation)*par.infl_redonyellow;
				}
				else if (c->Links[z->Sigma()] && c->tau == 2 && z->tau == 2){ 
					AvgX+= cos(z->orientation)*par.infl_yellowonyellow;
					AvgY+= sin(z->orientation)*par.infl_yellowonyellow;
				}
			}

      //Calculate the angle corresponding to AvgX and AvgY
			if (AvgX > 0 && AvgY > 0)
				AvgOrientation = atan(AvgY/AvgX);
			else if (AvgX < 0 && AvgY > 0)
				AvgOrientation = atan(-AvgX/AvgY) + PI/2;
			else if (AvgX < 0 && AvgY < 0)
				AvgOrientation = atan(AvgY/AvgX) + PI;
			else 
				AvgOrientation = atan(-AvgX/AvgY) + 3*PI/2;
		  //Assign a new polarization vector to cell c.
			double NewX, NewY;
			NewX = par.memory * cos(c->orientation) + (1-par.memory)* cos (AvgOrientation);
			NewY = par.memory * sin(c->orientation) + (1-par.memory)* sin (AvgOrientation);
			
      //Compute the angle corresponding to this vector.
			if (NewX > 0 && NewY > 0)
				c->orientation = atan(NewY/NewX);
			else if (NewX < 0 && NewY > 0)
				c->orientation = atan(-NewX/NewY) + PI/2;
			else if (NewX < 0 && NewY < 0)
				c->orientation = atan(NewY/NewX) + PI;
			else 
				c->orientation = atan(-NewX/NewY) + 3*PI/2;
...
//Differentiate cell type 1 to cell type2 with probability par.mean_difftime.
...
//energy from filipoids exerting a pulling force
int A = DH;
	if ( sxyp == MEDIUM ) {
		for (;c!=cell->end();c++) {
			if(c->Links[(*cell)[sxy].sigma] && c-> area !=0){
//cells to not always get apoptosed correctly. Do not try to pull non-exsistent cells. 

		DH -= (int)(par.lambda_force*(sqrt( 
		((*cell)[sxy].getCenterX()-c->getCenterX()) * ((*cell)[sxy].getCenterX()-c->getCenterX()) + 
		((*cell)[sxy].getCenterY()-c->getCenterY()) * ((*cell)[sxy].getCenterY()-c->getCenterY()))
		//distance between cell and c
		-sqrt(
		((*cell)[sxy].getCenterXIfXYWereRemoved(x,y)-c->getCenterX()) * 
		((*cell)[sxy].getCenterXIfXYWereRemoved(x,y)-c->getCenterX()) +
		((*cell)[sxy].getCenterYIfXYWereRemoved(x,y)-c->getCenterY()) * 
		((*cell)[sxy].getCenterYIfXYWereRemoved(x,y)-c->getCenterY()))));}
    //distance between cell and c if cell would shrink
		}
  }
  else if ( sxy == MEDIUM ) {
    for (;c!=cell->end();c++) {
			if(c->Links[(*cell)[sxyp].sigma] && c-> area !=0) {
    DH -= (int)(par.lambda_force*(sqrt( 
		((*cell)[sxyp].getCenterX()-c->getCenterX()) * ((*cell)[sxyp].getCenterX()-c->getCenterX()) + 
		((*cell)[sxyp].getCenterY()-c->getCenterY()) * ((*cell)[sxyp].getCenterY()-c->getCenterY()))
		//distance between cell and c
		-sqrt(
		((*cell)[sxyp].getCenterXIfXYWereAdded(x,y)-c->getCenterX()) * 
		((*cell)[sxyp].getCenterXIfXYWereAdded(x,y)-c->getCenterX()) +
		((*cell)[sxyp].getCenterYIfXYWereAdded(x,y)-c->getCenterY()) * 
		((*cell)[sxyp].getCenterYIfXYWereAdded(x,y)-c->getCenterY()))));}
    //distance between cell and c if cell would grow
		}

  }
  else {// calculate the distance difference if neither of the sites is the medium
		for (;c!=cell->end();c++) {
			if(c->Links[(*cell)[sxy].sigma] && c-> area !=0){
				DH -= (int)(par.lambda_force*(sqrt( 
				((*cell)[sxy].getCenterX()-c->getCenterX()) * ((*cell)[sxy].getCenterX()-c->getCenterX()) + 
				...
				//distance between cell and c
				-sqrt(
				((*cell)[sxy].getCenterXIfXYWereAdded(x,y)-c->getCenterX()) * 
				((*cell)[sxy].getCenterXIfXYWereAdded(x,y)-c->getCenterX()) +
				((*cell)[sxy].getCenterYIfXYWereAdded(x,y)-c->getCenterY()) * 
				((*cell)[sxy].getCenterYIfXYWereAdded(x,y)-c->getCenterY()))));}
			if(c->Links[(*cell)[sxyp].sigma] && c-> area !=0){
				DH -= (int)(par.lambda_force*(sqrt( 
				((*cell)[sxyp].getCenterX()-c->getCenterX()) * ((*cell)[sxyp].getCenterX()-c->getCenterX()) + 
				((*cell)[sxyp].getCenterY()-c->getCenterY()) * ((*cell)[sxyp].getCenterY()-c->getCenterY()))
				//distancebetween cell and c
				-sqrt(
				((*cell)[sxyp].getCenterXIfXYWereAdded(x,y)-c->getCenterX()) * 
				((*cell)[sxyp].getCenterXIfXYWereAdded(x,y)-c->getCenterX()) +
				((*cell)[sxyp].getCenterYIfXYWereAdded(x,y)-c->getCenterY()) * 
...
int CellularPotts::AmoebaeMove(PDE *PDEfield){
  ...
  for (int i; i<loop;i++){
    // take a random entry of the edgelist and calculate which sites belong to this edge.
      ...
      //Run administration on the neighbours of (x,y) to see if the edges corresponding to this neighbour should remain in the edgelist.
			for (int i = 1; i <= n_nb; i++){
				...
				if (xn>0 && yn>0 && xn<sizex-1 && yn<sizey-1){//if the neighbour site is within the lattice
					if (edgelist[edgeadjusting] == -1 && sigma[xn][yn] != sigma[x][y]){ //if an edge is not in the edgelist, yet the sites corresponding to it belong to different cells, add it
						AddEdgeToEdgelist(edgeadjusting);
						loop += (double)2/n_nb;
					}
					if (edgelist[edgeadjusting] != -1 && sigma[xn][yn] == sigma[x][y]){//if the sites corresponding to the edge belong to the same cell and they have an edge, remove it
...
void CellularPotts::AddEdgeToEdgelist(int edge) { //Administration to make sure the edgelist keeps the same form and is adjusted.
	int counteredge = CounterEdge(edge);	

	edgelist[edge] = sizeedgelist;
	orderedgelist[sizeedgelist] = edge;
	sizeedgelist++;
  //Also the edge the other way around should be added
	edgelist[counteredge] = sizeedgelist;
	orderedgelist[sizeedgelist] = counteredge;
	sizeedgelist++;
}

void CellularPotts::RemoveEdgeFromEdgelist(int edge) { //Administration to make sure the edgelist keeps the same form and is adjusted.
	int counteredge = CounterEdge(edge);	

	if(edgelist[edge] != sizeedgelist-1){ //if the edge is not last one in edgelist, make it the last one
  ...
  //remove the last edge
	...
	sizeedgelist--;
	if(edgelist[counteredge] != sizeedgelist-1){ //if counteredge is not last one in edgelist, make it the last one
...
int CellularPotts::CounterEdge(int edge){
  //This function computes which edge is between the same two sites as 'edge'
  ...
  //Find what neighbour is in the opposite direction as neighbourlocation.
  ...
  //Construct which edge is the edge the other way around
...
//compute the eccentricity of the tissue according to Zajac et al. 2003:
double CellularPotts::TissueEccentricity(void){
...
//Compute the circularity of the tissue
...
//If a cell is larger than size 90, divide it with probabilities par.mean_divtime_epi or par.mean_divtime_meso respectively
...
void CellularPotts::SetRandomTypes(void) {
  ...
  for (;
    ...
    //This makes sure we get a fraction or par.frac_yellow of yellow cells.
    c->setTau(celltype);
...
double CellularPotts::Compactness(double *res_compactness, double *res_area, double *res_cell_area) {
  ...
  // Calculate compactness of a tissue using the convex hull of the cells
