...
public:
  ...
  */
  ...
  inline int SizeY() const {
	...
	double TissueCircularity(void); //Calculates the ration of the circumference divided the circumference if the tissue were a sphere
	double TissueEccentricity(void); //Calculates how much a tissue is elongated with help of inertia moment
	double DegreeOfMixing(void); //Calculates sites neighbouring sites of another type divided by the total number of neighbours
  ...
  */
  void SetRandomTypes(void);
	...
	void RefreshLinks(void); //Refresh the links of all cells, as done by Belmonte et al. (2016)
	void Differentiation(void); //Allows cells to change cell types
...
private:
  ...
  void FreezeAmoebae(void);
	void AddEdgeToEdgelist(int edge); //An administrative function that makes sure that 'edgelist' and 'orderedgelist' get updated properly.
	void RemoveEdgeFromEdgelist(int edge); //An administrative function that makes sure that 'edgelist' and 'orderedgelist' get updated properly.
	int CounterEdge(int edge); //This function find the other edge that links the same two sites to each other.
